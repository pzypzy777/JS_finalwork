...
module.exports = {
    add: function (user, callback) { // users表中增加user操作
        ...
        pool.query("INSERT INTO user(name,password) VALUES (?,?);", sqlparam, function (error, result) {
    ...
    query: function (params, callback) { // 根据用户名密码查用户  登录
        let {
            name,
            pass
        } = params
    ...
    findByName: function (params, callback) { // 根据用户名查用户 
        let {
            name
        } = params
        let sqlparam = [name]
    ...
    findUsernameByID: function (params, callback) { // 根据用户名查用户 
        pool.query("SELECT name FROM user WHERE id = ?;", params, function (error, result) {
            if (error) throw error;
            callback(result[0]);
        });
    },
    ...
    update: function (params, callback) { // users表中更新user操作
        pool.query("UPDATE user set name= ?,password=? where id = ?;", [params.name, params.pass, params.id], function (error, result) {
            if (error) throw error;
            callback(result);
        });
    },
